...
- [x] App (đã có D1 + session token auth + edit profile + logout + profile settings nâng cao + quản trị phiên theo session + audit logs + rate-limit + contract/rate-limit smoke tests)  
